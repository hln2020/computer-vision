...
def evaluate_pred(config):
    ...
    if config.model=='FutureGAN' or config.model=='CopyLast':
        ...
        #original end
        #hailie begin
        #dataset_gt = VideoFolder(video_root=test_data_root, video_ext=config.ext, nframes=nframes_in, loader=video_loader, transform=transform)
        #hailie end
    ...
    #original end
    #hailie begin
    #x = Variable(torch.FloatTensor(config.batch_size, config.nc, nframes_in, img_size, img_size))
    #hailie end
    ...
    for step in tqdm(range(len(data_iter_gt))):
        ...
        # save frames
        print("save frame every: ", config.save_frames_every)
        if config.save_frames_every != 0 and config.model=='FutureGAN':
            #hailie begin
            print(step)
            #hailie end
